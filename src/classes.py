...
class VelocityField():
    ...
    def __init__(self, file_path="/", time_step=0, mean_file_path="/",file_type="/"):
        ...
        ## Create a mesh
        #self.samples = self.u_velocity_matrix.shape[1]
		...
		# Default: data are x, y, z, u, v, w
            ...
            index_x,index_y,index_z,index_u,index_v,index_w = 0,1,2,3,4,5     
            datafile_read=np.loadtxt(self.file_path,delimiter=" ",dtype=float,skiprows=3) #skip header, default is 3 lines
            ...
            self.u_velocity_matrix  = np.array(datafile_read[:,index_u]).reshape(self.x_coordinate_size,self.y_coordinate_size)
            self.v_velocity_matrix  = np.array(datafile_read[:,index_v]).reshape(self.x_coordinate_size,self.y_coordinate_size)
            self.w_velocity_matrix  = np.array(datafile_read[:,index_w]).reshape(self.x_coordinate_size,self.y_coordinate_size)
            ...
            if (self.mean_file_path != '/' ):
                ...
                u_velocity_matrixMean  = np.array(datafile_mean_read[:,index_u]).reshape(self.x_coordinate_size,self.y_coordinate_size)
                v_velocity_matrixMean  = np.array(datafile_mean_read[:,index_v]).reshape(self.x_coordinate_size,self.y_coordinate_size)
                w_velocity_matrixMean  = np.array(datafile_mean_read[:,index_w]).reshape(self.x_coordinate_size,self.y_coordinate_size)
                self.u_velocity_matrix = self.u_velocity_matrix - u_velocity_matrixMean
                self.v_velocity_matrix = self.v_velocity_matrix - v_velocity_matrixMean
                self.w_velocity_matrix = self.w_velocity_matrix - w_velocity_matrixMean
        ...
        #COMMON TO ALL DATA
        self.x_coordinate_step=round((np.max(self.x_coordinate_matrix)-np.min(self.x_coordinate_matrix)) / (np.size(self.x_coordinate_matrix)-1) ,6)
        self.y_coordinate_step=round((np.max(self.y_coordinate_matrix)-np.min(self.y_coordinate_matrix)) / (np.size(self.y_coordinate_matrix)-1) ,6)
