...
if __name__ == '__main__':
    ...
    print('Some tests for the Rankine model')
    print(args.infilename)

    vfield = VelocityField(args.infilename, 0, '/', 'PIV NetCDF (.nc)')


    def test_rankine(core_radius, gamma, dist, xdrift, ydrift, u_advection, v_advection,theoretical_model):
        ...
        u_data, v_data = fitting.velocity_model(core_radius, gamma, x_real, y_real,
                                                u_advection, v_advection, x_index + xdrift, y_index + ydrift, None, 
                                                theoretical_model)
        ...
        model = fitting.fit(core_radius, gamma, x_index, y_index, x_real, y_real, u_data, v_data, None, None, u_advection,v_advection, 0, theoretical_model)
        ...
        u_model, v_model = fitting.velocity_model(model[0], model[1], model[2], model[3],
                                                  model[4], model[5], x_index, y_index, None, theoretical_model)
        corr = fitting.correlation_coef(u_data, v_data, None, u_model, v_model, None)
    ...
    test_rankine(0.2, 10, 10, 0.0, 0.0, 0.01, 0.01, 'rankine')
    test_rankine(0.2, 10, 10, 0.2, 0.2, 0.01, 0.01, 'rankine')
    test_rankine(0.9, 40, 10, 0.0, 0.0, 0.01, 0.01, 'rankine')
    test_rankine(0.9, 40, 10, 0.2, 0.2, 0.01, 0.01, 'rankine')
